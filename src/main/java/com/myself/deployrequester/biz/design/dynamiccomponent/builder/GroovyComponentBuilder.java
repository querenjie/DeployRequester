...
public class GroovyComponentBuilder implements ComponentBuilder {
	...
	/**
	 * GroovyClassLoader有innerClassLoader的机制，
	 * 实际上加载的每个class是由一个独立的innerclassloader加载的
	...
	private <T> T load(Class<T> clazz, ResourceFile source) {
		try {
			Class<?> loadedClazz = groovyClassLoader.parseClass(new String(
					source.getContent(), encoding), source.getName());
			Assert.notNull(loadedClazz, "parse class fail " + clazz.getName());
			Object instance = loadedClazz.newInstance();
			Assert.isInstanceOf(clazz, instance, "isInstance fail "
					+ clazz.getName());
			// 清除cache，这样当class没有被引用的时候才会被释放掉
			groovyClassLoader.clearCache();
			return (T) instance;
		} catch (Exception e) {
			throw new IllegalStateException("groovy load fail "
					+ clazz.getName(), e);
		}
	}
